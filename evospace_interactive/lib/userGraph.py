...
graph_db = neo4j.GraphDatabaseService("http://evospaceInteractive:4gjMcrCzICFtUcL2ZCmG@evospaceinteractive.sb02.stations.graphenedb.com:24789/db/data/")
#local  ne4j
...
class Graph_Individual(object):
    ...
    def get_node(self, id=None, ind_status=None):
        self.id = id
        self.status = ind_status
        try:

            #query = "MATCH (n) WHERE n.id='" + self.id + "' AND n.ind_status='" + self.status + "'return n"
